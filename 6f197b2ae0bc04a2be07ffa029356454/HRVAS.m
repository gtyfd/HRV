...
%   -add error handling
    ...
    figframe=get(h.MainFigure,'javaframe');
    Icon=javax.swing.ImageIcon('./hrvas_icon.png');
    figframe.setFigureIcon(Icon);
...
%% GUI: Results - Time Domain Tab
    ...
    h.tab1 = uitab('parent',h.tabgroup, 'title', '时域分析');
...
%% GUI: Results - Freq Doamin Tab
    ...
    h.tab2 = uitab(h.tabgroup, 'title', '频域分析');
    h.panelFreq = uipanel('Parent',h.tab2, ...
        'Position',[.0 .0 1 1],'BackgroundColor','white');
    h.axesFreq = axes('parent', h.panelFreq, ...
        'Position',[.1 .57 .8 .36], 'FontSize',7,'Box','on');
    xlabel(h.axesFreq, '频率 (Hz)','FontSize',8); 
    ylabel(h.axesFreq, '功率谱密度 (ms^2/Hz)','FontSize',8);
    ...
    h.lblFreqPlot = uicontrol(h.btngrpFreqPlot,'Style', ...
        'text','String','方法 :', 'Units','normalized', ...
...
%% GUI: Results - Poincare Tab
    ...
    h.tab3 = uitab('Parent',h.tabgroup, 'title', '庞加莱图');
...
%% GUI: Results - TimeFreq Tab
    ...
    % Create Type button group.
     h.listTFPlot = uicontrol(h.panelTF,'Style','popupmenu',...
        'String',{'频谱图', '频谱图 (log)','三维曲面图', ...
        '瀑布图', '全局 功率谱密度','低频/高频 比例图','低频 & 高频 功率图'}, ...
...
%% Helper and Utility Functions
    ...
    % createTimeTbl: creats the table to display time-domain HRV results    
        ...
        text(0,1,'时域分析','Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',8)
        %Col Headers
        text(x1-.02,.9,'统计学度量','Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',8)
        text(x2,.9,'单位','Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8)
        text(x3,.9,'值','Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8)
        %Column 1
        tH(1,1)=text(x1,y(1),'平均IBI','Parent',aH,'Units','normalized');%MeanIBI
        tH(2,1)=text(x1,y(2),'IBI标准差','Parent',aH,'Units','normalized');
        tH(3,1)=text(x1,y(3),'平均HR','Parent',aH,'Units','normalized');
        tH(4,1)=text(x1,y(4),'HR标准差','Parent',aH,'Units','normalized');
        ...
        tH(9,1)=text(x1-.02,y(10),'几何学度量','Parent',aH, ...
            'Units','normalized','fontweight','bold');
        tH(10,1)=text(x1,y(11),'HRV 三角指数','Parent',aH,...
            'Units','normalized');
...
%% GUI: CreateFreqTbl
    function tH = createFreqTbl(aH)
        ...
        line([0 1],[0 0],'Parent',aH,'Color','black');      
        %Header
        text(x1-.02,.88,{'频域度量','频带'},'Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',8);
        text(x2,.88,{'峰值','(Hz)'},'Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right','fontweight','bold','FontSize',8);
        text(x3,.88,{'功率','(ms^2)'},'Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8);        
        text(x4,.88,{'功率','(%)'},'Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8);
        text(x5,.88,{'功率','(n.u.)'},'Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8);
        text(x6,.88,{'低频/高频','(比例)'},'Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',8,  ...
            'HorizontalAlignment','right');
        %Column 1
        tH(1,1)=text(x1-.02,y(1),'Welch 功率谱密度','Parent',aH, ...
            'FontWeight','bold');
        tH(2,1)=text(x1,y(2),'超低频','Parent',aH,'Units','normalized');
        tH(3,1)=text(x1,y(3),'低频','Parent',aH,'Units','normalized');
        tH(4,1)=text(x1,y(4),'高频','Parent',aH,'Units','normalized');
        tH(5,1)=text(x1-.02,y(5),'Burg 功率谱密度','Parent',aH, ...
            'Units','normalized', 'FontWeight','bold');
        tH(6,1)=text(x1,y(6),'超低频','Parent',aH,'Units','normalized');
        tH(7,1)=text(x1,y(7),'低频','Parent',aH,'Units','normalized');
        tH(8,1)=text(x1,y(8),'高频','Parent',aH,'Units','normalized');
        tH(9,1)=text(x1-.02,y(9),'Lomb-Scargle 功率谱密度 (标准化)','Parent',aH, ...
            'Units','normalized', 'FontWeight','bold');
        tH(10,1)=text(x1,y(10),'超低频','Parent',aH,'Units','normalized');
        tH(11,1)=text(x1,y(11),'低频','Parent',aH,'Units','normalized');
        tH(12,1)=text(x1,y(12),'高频','Parent',aH,'Units','normalized');
...
%% GUI: CreateNLTbl
    function tH = createNLTbl(aH)
        ...
        line([0 1],[.85 .85],'Parent',aH,'Color','black')
        line([0 1],[0 0],'Parent',aH,'Color','black')
        text(0,1,'非线性统计','Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',10)
        %Col Headers
        text(x1-.02,.9,'度量','Parent',aH,'Units','normalized' ...
        ...
        text(x3,.9,'值','Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',10)
        %Column 1
        tH(1,1)=text(x1-.02,.75,'熵值','Parent',aH, ...
            'Units','normalized', 'fontweight','bold');
        tH(2,1)=text(x1,.65,'样本熵','Parent',aH,'Units', ...
            'normalized');
        tH(3,1)=text(x1-.02,.55,'去趋势波动分析','Parent',aH,'Units', ...
        ...
        set(findobj(tH,'flat','string','0.00','parent',aH), ...
            'HorizontalAlignment','right')
        set(findobj(tH,'type','text','parent',aH),'Fontsize',10)
    end
%% GUI: CreateTFTbl
    function tH = createTFTbl(aH)        
        ...
        line([0 1],[0 0],'Parent',aH,'Color','black')        
        %Header
        text(x1-.02,.88,{'频率度量','频带'},'Parent',aH, ...
            'Units','normalized', 'fontweight','bold','FontSize',8)
        text(x2,.88,{'峰值','(Hz)'},'Parent',aH,'Units','normalized', ...
           'HorizontalAlignment','right', 'fontweight','bold','FontSize',8)
        text(x3,.88,{'功率','(ms^2)'},'Parent',aH,'Units','normalized', ...
        ...
        text(x5,.88,{'功率','(n.u.)'},'Parent',aH,'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8)
        text(x6,.88,{'低频/高频','(比率)'},'Parent',aH, ...
            'Units','normalized', ...
            'HorizontalAlignment','right', 'fontweight','bold','FontSize',8)
        %Column 1
        tH(1,1)=text(x1-.02,y(1),'Burg 功率谱密度','Parent',aH, ...
            'Units','normalized', 'FontWeight','bold');
        tH(2,1)=text(x1,y(2),'超低频','Parent',aH,'Units','normalized');
        tH(3,1)=text(x1,y(3),'低频','Parent',aH,'Units','normalized');
        tH(4,1)=text(x1,y(4),'高频','Parent',aH,'Units','normalized');
        tH(5,1)=text(x1-.02,y(5),'Lomb-Scargle 功率谱密度','Parent',aH, ...
            'Units','normalized','FontWeight','bold');
        tH(6,1)=text(x1,y(6),'超低频','Parent',aH,'Units','normalized');
        tH(7,1)=text(x1,y(7),'低频','Parent',aH,'Units','normalized');
        tH(8,1)=text(x1,y(8),'高频','Parent',aH,'Units','normalized');
        tH(9,1)=text(x1-.02,y(9),'Wavelet 功率谱密度','Parent',aH, ...
            'Units','normalized', 'FontWeight','bold');
        tH(10,1)=text(x1,y(10),'超低频','Parent',aH,'Units','normalized');
        tH(11,1)=text(x1,y(11),'低频','Parent',aH,'Units','normalized');
        tH(12,1)=text(x1,y(12),'高频','Parent',aH,'Units','normalized');
    ...
    % plotFreq: plots freq-domain related figures
        ...
        set(h.axesFreq,'FontSize',7);
        xlabel(h.axesFreq,'频率 (Hz)','FontSize',9); 
...
%% Helper: Plot - NL
    function plotNL(h,hrv,opt)
        ...
        set(h.axesNL,'xlim',xlim,'ylim',ylim,'FontSize',8)
        title(h.axesNL,'去趋势波动分析','FontSize',12)
        xlabel(h.axesNL,'log_1_0 n','FontSize',9)
        ylabel(h.axesNL,'log_1_0 F(n)','FontSize',9)
    ...
    function plotTF(h,hrv,opt)
        ...
        switch lower(pt)
            ...
            case '全局 功率谱密度'                
                ...
                xlabel(h.axesTF,'频率 (Hz)');
                ylabel(h.axesTF,'全局 功率谱密度 (s^2/Hz)');
            case '低频 & 高频 功率图'
